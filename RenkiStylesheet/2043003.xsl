...
<xsl:template match="/">
	<html>
		<head>
			...
			<title>厚生年金保険被保険者種別変更確認一覧表</title>
			<style type="text/css">
			<!--テーブル属性-->
				td {
			...
			<td class="outline">
						...
						<td style="vertical-align:top;padding:0px">
							...
							<!-- タイトル作成・被保険者証の記号表示 -->
								<tr>
									...
									<td class="title" colspan="11">　　　　厚生年金保険被保険者種別変更確認一覧表</td>
								</tr>
								
								<tr class="h_data1">
									<td class="normalct allline"><xsl:value-of select="事業所整理記号x郡市区記号" /></td>
									<td class="normalct allline" rowspan="2"><xsl:value-of select="事業所整理記号x記号" /></td>
									<td class="normalct lineb" colspan="11" rowspan="2"></td>	<!-- ｢被　保　険　者　の　氏　名｣上の罫線が何故か消える為 -->
								</tr>
								
								<tr class="h_data1">
									<td class="normalct allline"><xsl:value-of select="事業所整理記号x郡市区符号" /></td>
								</tr>
							...
							<!--　被保険者単位のデータ表示　-->
								...
								<tr>
									...
									<td class="midashict allline" colspan="2" >被　保　険　者　の　氏　名</td>
									<td class="midashict allline">生年月日</td>
									<td class="midashict allline">変更後<br/>の種別</td>
									<td class="midashict allline" colspan="2">基礎年金番号</td>
									<td class="midashict allline"  colspan="4">変更年月日</td>
									<td class="midashict allline">変更前<br/>の種別</td>
				...
				<table class="detail" cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<td class="normalrt" rowspan="2" valign="top"></td>
					</tr>

					<tr class="h_data2">
						<td class="normal linel" valign="top" ><xsl:value-of select="事業所所在地x住所" /></td>	<!-- 2003/0911 -->
						<td class="linel"><br/></td>
					</tr>
					...
					<tr>
						...
						<td class="normal linel">
							<xsl:if test="電話番号x市外局番='' and 電話番号x局番='' and 電話番号x番号=''">
							</xsl:if>
...
<xsl:template match="_被保険者">
	...
	<tr>
		<td colspan="13"><br class="small" /></td>
	</tr>
	
	<tr class="h_data4">
		<td class="normalct allline"><xsl:value-of select="被保険者整理番号" /></td>
		<td class="normal allline" colspan="2"><xsl:value-of select="被保険者の氏名" /></td>
		...
		<td class="normalct allline"><xsl:value-of select="変更後の種別" /></td>
		<td class="normalct allline"><xsl:value-of select="基礎年金番号x課所符号" /></td>
		<td class="normalct allline"><xsl:value-of select="基礎年金番号x一連番号" /></td>
		<td class="normalrt allline linerclear linelclear"><xsl:value-of select="変更年月日x元号" /></td>
		<td class="normalrt allline linerclear linelclear"><xsl:value-of select="concat(変更年月日x年,'年')" /></td>
		<td class="normalrt allline linerclear linelclear"><xsl:value-of select="concat(変更年月日x月,'月')" /></td>
		<td class="normalrt allline linelclear"><xsl:value-of select="concat(変更年月日x日,'日')" /></td>
		<td class="normalct allline"><xsl:value-of select="変更前の種別" /></td>
