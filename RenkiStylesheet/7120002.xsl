	...
	<xsl:template match="_被保険者" mode="hihokensha">
		...
		<xsl:if test="($pagecnt_1=ceiling(position() div 15))">
			...
			<tr>
				...
				<td class="normalM_L linelb" style="padding-left:1px;"><xsl:value-of select="基礎年金番号_上4桁" /></td>
			...
			<table class="detail" cellpadding="1" cellspacing="0">
				<td style="vertical-align:top;">
					<table class="detail" >
						...
						<tr>
							<td></td>
							<td class="normalS_L" style="height:25px">※2　喪失原因　　　4（他）：退職等　5（死亡）：死亡　6（70）：70歳到達による厚生年金保険のみ喪失<br/>　　　　　　　　　 7（75）：75歳到達による健康保険のみ喪失　9（障害）：障害認定　3（快復）：障害の快復<br/>　　　　　　　　　 11（協定）：社会保障協定による喪失<br/>　　　　　　　　　 ※喪失原因コードは先頭に0が付与される場合あり（例:04（他）：退職等）</td>
						</tr>
						<tr>
							<td></td>
