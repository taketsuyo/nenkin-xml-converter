	...
	<xsl:template match="/">
		<html>
			<head>
				...
				<title>健康保険・厚生年金保険標準賞与額決定一覧表</title>
				<style type="text/css">
				<!--テーブル属性-->
					table.outline						<!--外枠-->
					{
					...
					<table class="detail" cellpadding="0" cellspacing="0">
						<td style="vertical-align:top;">
							...
							<table class="Lterritory" >
								...
								<tr>
									<td></td>
									<td class="normalM_TL" style="height:14px;"><xsl:value-of select="事業所所在地_送付先" /></td>
									<td></td>
								</tr>
								<tr>
									<td></td>
									<td class="normalM_TL" style="height:14px;"><xsl:value-of select="事業所所在地_送付先1" /></td>
									<td></td>
								</tr>
								<tr>
									<td></td>
						...
						<td style="vertical-align:top;">
							...
							<table class="Rterritory" cellpadding="3" cellspacing="0">
								...
								<tr>
									<td></td>
									<td class="normalS_TL allline" style="height:185px;line-height:11px;"><pre class="oshirase">この表は被保険者ごとに送付された「健康保険・厚生年金保険標準賞与額決定通知書」の内容を一覧にしています。
なお、欄外に※の表示がある記録は、過去の標準賞与額について登録を行ったことにより、その後の標準賞与額が、年度における標準賞与額の累計額の上限を超えたため減額訂正した記録となります。</pre></td>
									<td></td>
								</tr>
							</table>
					...
					<table class="detail">
						<tr>
							<td class="title" style="height:50px; padding-top:10px;">健康保険・厚生年金保険標準賞与額決定一覧表</td>
						</tr>
					</table>
				...
				<td style="vertical-align:top;">
					<table class="detail" cellpadding="0" cellspacing="0">
						...
						<tr>
							...
							<td class="midashiS_C linetrb" colspan="2">決定後の標準賞与額</td>
							<td class="midashiS_L linetlr" style="padding-left:6px;" colspan="6">※1</td>
						...
						<tr>
							<td></td>
							<td class="midashiS_C linelrb" colspan="6">賞与支払年月日</td>
	...
	<xsl:template match="_被保険者" mode="hihokensha">
		...
		<xsl:if test="($pagecnt_1=ceiling(position() div 15))">
			...
			<tr>
				...
				<td class="normalM_L linelrb" style="padding-left:12px;"><xsl:value-of select="被保険者氏名" /></td>
				<td class="normalL_L linelb" style="padding-left:6px;"><xsl:value-of select="賞与支払年月日_元号" /></td>
				<xsl:if test="賞与支払年月日_年!=''">
					<td class="normalL_R lineb">
						<xsl:call-template name="zero-padding">
							<xsl:with-param name="str" select="賞与支払年月日_年"/>
						</xsl:call-template>
					</td>
				</xsl:if>
				<xsl:if test="賞与支払年月日_年=''">
				...
				<xsl:if test="賞与支払年月日_月!=''">
					<td class="normalL_R lineb">
						<xsl:call-template name="zero-padding">
							<xsl:with-param name="str" select="賞与支払年月日_月"/>
						</xsl:call-template>
					</td>
				</xsl:if>
				<xsl:if test="賞与支払年月日_月=''">
				...
				<td class="midashiL_L lineb">.</td>
				<xsl:if test="賞与支払年月日_日!=''">
					<td class="normalL_R linerb" style="padding-right:6px;">
						<xsl:call-template name="zero-padding">
							<xsl:with-param name="str" select="賞与支払年月日_日"/>
						</xsl:call-template>
					</td>
				</xsl:if>
				<xsl:if test="賞与支払年月日_日=''">
				...
				<td class="normalL_R linerb" style="padding-right:2px;"><xsl:value-of select="決定後の標準賞与額_健保" /></td>
				<td class="normalL_R linerb" style="padding-right:2px;"><xsl:value-of select="決定後の標準賞与額_厚年" /></td>
				<td class="normalL_L lineb" style="padding-left:6px;"><xsl:value-of select="生年月日_元号" /></td>
				<xsl:if test="生年月日_年!=''">
					<td class="normalL_R lineb">
						<xsl:call-template name="zero-padding">
				...
				<td class="normalM_L linelrb" style="padding-left:6px;"><xsl:value-of select="種別" /></td>
				<xsl:if test="機構からのお知らせ='健康保険では、年度（毎年４月１日から翌年３月３１日まで）における標準賞与額の累計額に上限が定められています。　　　　　　　　　　　　　　　　　　　　　　　　下記の被保険者については、過去の標準賞与額について登録、訂正又は取消（以下本欄において「登録等」といいます。）を行いました。併せて、その登録等の日後の標準賞与額について、年度における標準賞与額の累計額の上限を超えたため減額訂正、又は上限を超えないこととなったため増額訂正しました。'">
					<td class="midashiM_L" style="padding-left:6px;">※</td>
				</xsl:if>
				<xsl:if test="機構からのお知らせ='通知書は、処理が完了した方の分から順次発送しております。すでにご提出済みの方が今回の通知書に含まれていない場合、処理が完了次第、送付いたします。'">
					<td class="midashiM_L" style="padding-left:6px;">　</td>
				</xsl:if>
				<td></td>
			</tr>
			...
			<table class="detail" cellpadding="1" cellspacing="0">
				<td style="vertical-align:top;">
					<table class="detail" >
						...
						<tr>
							<td></td>
							<td class="normalS_L" style="height:25px">※2　種別　　第一種：男性　第二種：女性　第三種：坑内員　特例第一種：男性（基金加入）　特例第二種：女性（基金加入）<br/>　　　　　　 特例第三種：坑内員（基金加入）</td>
						</tr>
						<tr>
							<td></td>
							<td class="normalS_L" style="height:30px;padding-left:15px;"></td>
						</tr>
					</table>
				</td>
			</table>
