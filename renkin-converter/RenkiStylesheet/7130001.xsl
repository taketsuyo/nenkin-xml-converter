	...
	<xsl:template match="/">
		<html>
			<head>
				...
				<title>健康保険・厚生年金保険被保険者標準報酬決定一覧表</title>
				<style type="text/css">
				<!--テーブル属性-->
					table.outline						<!--外枠-->
					{
		...
		<xsl:if test="($pagecnt_1=position())">
			<table class="detail" cellpadding="0" cellspacing="0">
				<tr>
					...
					<table class="detail" cellpadding="0" cellspacing="0">
						<tr>
							...
							<table class="Lterritory" >
								...
								<tr>
									<td></td>
									<td class="normalM_TL" style="height:12px;"><xsl:value-of select="事業所所在地_送付先" /></td>
									<td></td>
								</tr>
								<tr>
									<td></td>
									<td class="normalM_TL" style="height:12px;"><xsl:value-of select="事業所所在地_送付先1" /></td>
									<td></td>
								</tr>
								<tr>
									<td></td>
						...
						<td style="vertical-align:top;">
							...
							<table class="Rterritory" cellpadding="0" cellspacing="0">
								...
								<tr>
									<td></td>
									<td class="normalS_RG" style="height:10px"><xsl:value-of select="SC_固定文字" /></td>
									<td></td>
					...
					<table class="detail">
						<tr>
							<td class="title" style="height:50px; padding-top:10px;">健康保険・厚生年金保険被保険者標準報酬決定一覧表</td>
						</tr>
					</table>
	...
	<xsl:template match="_被保険者" mode="hihokensha">
		...
		<xsl:if test="($pagecnt_1=ceiling(position() div 15))">
			<tr>
				...
				<td class="normalL_L linelb" style="padding-left:6px;"><xsl:value-of select="適用年月_元号" /></td>
				<xsl:if test="適用年月_年!=''">
					<td class="normalL_R lineb">
						<xsl:call-template name="zero-padding">
							<xsl:with-param name="str" select="適用年月_年"/>
						</xsl:call-template>
					</td>
				</xsl:if>
				<xsl:if test="適用年月_年=''">
				...
				<xsl:if test="適用年月_月!=''">
					<td class="normalL_R linerb" style="padding-right:6px;">
						<xsl:call-template name="zero-padding">
							<xsl:with-param name="str" select="適用年月_月"/>
						</xsl:call-template>
					</td>
				</xsl:if>
				<xsl:if test="適用年月_月=''">
				...
				<td class="normalL_R linerb" style="padding-right:6px;"><xsl:value-of select="決定後の標準報酬月額_健保" /></td>
				<td class="normalL_R linerb" style="padding-right:6px;"><xsl:value-of select="決定後の標準報酬月額_厚年" /></td>
			...
			<table class="detail" cellpadding="1" cellspacing="0">
				<tr>
					...
					<table class="detail" >
						...
						<tr>
							<td></td>
							<td class="normalM_L" style="height:25px">※2　種別　第一種：男性　第二種：女性　第三種：坑内員　特例第一種：男性（基金加入）　特例第二種：女性（基金加入）<br/>　　　　　 特例第三種：坑内員（基金加入）</td>
						</tr>
						<tr>
							<td></td>
